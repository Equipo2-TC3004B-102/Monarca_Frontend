...
const Mosaic = ({ title, iconPath, link, id}: MosaicProps) => {
    ...
    return (
        <Link to={link} data-cy={`mosaic-${title.toLowerCase().replace(/\s+/g, '-')}`}>
          <div
          className="relative bg-[var(--color-card-bg)] w-64 h-30 rounded-2xl shadow-md flex items-end justify-center hover:shadow-lg transition-shadow duration-300 ease-in-out"
          id={id ? id : undefined}
        >
